...
export const JsonTableView: React.FC<JsonTableViewProps> = ({ value, searchTerm = '' }) => {
  ...
  const renderCell = (cellValue: any, onClickPath?: (string | number)[]) => {
    ...
    if (cellValue === undefined) return <span className="text-muted-foreground text-[10px] italic"></span>;
    ...
    if (isObj) {
       ...
       return (
        <button
            ...
            className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-border hover:bg-muted-foreground text-foreground text-[11px] font-medium transition-colors group border border-transparent hover:border-muted-foreground"
        >
           <FolderOpen size={10} className="text-muted-foreground group-hover:text-foreground" />
           {label}
        </button>
       );
    }
    ...
    if (str.length > 80) {
        return <span className="text-foreground" title={str}>{str.substring(0, 80)}...</span>;
    }
  ...
  const Breadcrumbs = () => (
    <div className="flex items-center gap-1 px-4 py-2 border-b border-border bg-muted overflow-x-auto whitespace-nowrap scrollbar-hide shrink-0 h-10">
        <button
            onClick={handleReset}
            className={`p-1 rounded hover:bg-muted-foreground transition-colors ${path.length === 0 ? 'text-foreground font-semibold' : 'text-muted-foreground'}`}
            title="Root"
        >
            <Home size={14} />
        </button>
        {path.map((segment, idx) => (
            <React.Fragment key={idx}>
                ...
                <button
                    ...
                    className={`px-1.5 py-0.5 rounded text-xs hover:bg-muted-foreground transition-colors ${idx === path.length - 1 ? 'text-foreground font-semibold bg-border' : 'text-muted-foreground'}`}
                >
                    {segment}
                </button>
            </React.Fragment>
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center text-destructive gap-4 p-8 text-center bg-background">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm max-w-md">Please fix errors in Code view.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full w-full bg-background">
        ...
        <div className="flex-1 overflow-auto bg-background relative">
            {currentType === 'array' && (() => {
                const arr = currentData as any[];
                if (arr.length === 0) return <div className="h-full flex items-center justify-center text-muted-foreground text-sm">Empty Array</div>;

                const filteredArr = searchTerm
                    ? arr.filter(item => JSON.stringify(item).toLowerCase().includes(searchTerm.toLowerCase()))
                    : arr;

                if (filteredArr.length === 0) return <div className="h-full flex flex-col items-center justify-center text-muted-foreground gap-2"><Filter size={24}/><span>No matches</span></div>;

                return (
                    <table className="w-full text-left border-collapse">
                    <thead className="sticky top-0 z-10">
                        <tr className="bg-muted shadow-sm">
                        <th className="p-2 border-b border-border font-medium text-muted-foreground text-[11px] uppercase tracking-wider w-12 text-center sticky left-0 bg-muted border-r">#</th>
                        {headers.map(h => (
                            <th key={h} className="p-2 border-b border-border font-medium text-muted-foreground text-[11px] uppercase tracking-wider min-w-[120px] whitespace-nowrap">{h}</th>
                        ))}
                        </tr>
                    </thead>
            ...
            {currentType === 'object' && (() => {
                ...
                if (keys.length === 0) return <div className="h-full flex items-center justify-center text-muted-foreground text-sm">Empty Object</div>;
                ...
                if (filteredKeys.length === 0) return <div className="h-full flex flex-col items-center justify-center text-muted-foreground gap-2"><Filter size={24}/><span>No matches</span></div>;
                        ...
                        <div className="w-full max-w-3xl border border-border rounded-lg overflow-hidden shadow-sm bg-background">
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-muted">
                                    <tr>
                                        <th className="p-2 border-b border-border font-medium text-muted-foreground text-[11px] uppercase tracking-wider w-1/3 border-r">Key</th>
                                        <th className="p-2 border-b border-border font-medium text-muted-foreground text-[11px] uppercase tracking-wider">Value</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-border">
                                    {filteredKeys.map(key => (
                                        <tr key={key} className="hover:bg-muted/50 transition-colors">
                                            <td className="p-2 font-semibold text-foreground font-mono text-xs align-top border-r border-border select-text">{key}</td>
            ...
            {currentType === 'primitive' && (
                <div className="h-full flex flex-col items-center justify-center p-8 bg-background">
                    <div className="bg-muted p-8 rounded-xl border border-border shadow-sm text-center max-w-md w-full relative">
                        <button onClick={() => setPath(prev => prev.slice(0, -1))} className="absolute top-4 left-4 p-1 hover:bg-border rounded text-muted-foreground">
                             <ArrowLeft size={16} />
                        </button>
                        <div className="mb-4 text-muted-foreground text-xs uppercase tracking-wider font-semibold border-b border-border pb-2">Value</div>
                        <div className="text-xl font-mono text-foreground break-all select-all max-h-60 overflow-y-auto">{String(currentData)}</div>
                         <div className="mt-4 pt-2 text-[10px] text-muted-foreground border-t border-border flex justify-between">
                            <span>Type: <span className="font-semibold text-foreground">{typeof currentData}</span></span>
                         </div>
                    </div>
                </div>
            )}
