...
export const JsonMermaidView: React.FC<JsonMermaidViewProps> = ({ value, theme }) => {
  ...
  return (
    <div className="relative w-full h-full bg-background overflow-hidden">
        {/* Toolbar */}
        <div className="absolute top-4 right-4 flex flex-col gap-2 z-20 bg-muted/95 border border-border p-1.5 rounded-lg shadow-xl backdrop-blur-md">
           <div className="flex flex-col gap-1 border-b border-border pb-2 mb-1">
             <button 
                ...
                className={`flex items-center gap-2 p-2 rounded text-xs font-medium transition-colors ${diagramType === 'mindmap' ? 'bg-foreground text-background' : 'text-muted-foreground hover:bg-border'}`}
             >
                <GitFork size={14} /> Mindmap
             </button>
             <button 
                onClick={() => { setDiagramType('graph'); trackEvent('diagram_switch', { type: 'graph' }); }}
                className={`flex items-center gap-2 p-2 rounded text-xs font-medium transition-colors ${diagramType === 'graph' ? 'bg-foreground text-background' : 'text-muted-foreground hover:bg-border'}`}
             >
                <Workflow size={14} /> Flowchart
             </button>
           </div>
           
           <button onClick={() => handleZoom(0.2)} className="p-2 hover:bg-muted-foreground rounded text-muted-foreground hover:text-foreground" title="Zoom In"><ZoomIn size={16} /></button>
           <button onClick={() => handleZoom(-0.2)} className="p-2 hover:bg-muted-foreground rounded text-muted-foreground hover:text-foreground" title="Zoom Out"><ZoomOut size={16} /></button>
           <button onClick={() => { setScale(1); setPosition({x:0, y:0}); }} className="p-2 hover:bg-muted-foreground rounded text-muted-foreground hover:text-foreground" title="Reset"><RotateCcw size={16} /></button>
        </div>
