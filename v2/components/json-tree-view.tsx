...
const GraphNode: React.FC<{ 
  ...
  const TypeIcon = () => {
    switch (type) {
      ...
      default: return <div className="w-3.5 h-3.5 rounded-full bg-muted-foreground" />;
    }
  };
  ...
  const renderValue = () => {
    if (isExpandable) {
      ...
      return <span className="text-muted-foreground text-[10px] ml-2">{len} {type === 'array' ? 'items' : 'keys'}</span>;
    }
      ...
      <div className="flex flex-col items-start z-10">
        <div 
          ...
          className={`
            ...
            ${isMatch ? 'bg-yellow-900/30 border-yellow-500/50 graph-search-match' : 'bg-muted border-border hover:bg-border'}
          `}
        ...
        >
          ...
          <div className="flex flex-col">
            <div className="flex items-center">
              {name && <span className="font-semibold text-xs text-foreground mr-1">{name}</span>}
              {!name && <span className="font-semibold text-xs text-muted-foreground italic">root</span>}
            </div>
          </div>
            ...
            {hasMore && (
              <div className="flex items-start pt-2 pb-2 pl-4 relative">
                 ...
                 <button 
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-dashed border-muted-foreground bg-transparent hover:bg-border hover:border-muted-foreground text-xs text-muted-foreground transition-colors"
                 >
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-border pb-2">
        <span className="font-semibold text-foreground break-all">{data.name || 'root'}</span>
      ...
      <div className="flex flex-col gap-1">
        <span className="text-muted-foreground text-[10px] uppercase tracking-wider">Path</span>
      ...
      <div className="flex flex-col gap-1">
        <span className="text-muted-foreground text-[10px] uppercase tracking-wider">Value</span>
...
export const JsonTreeView: React.FC<JsonGraphViewProps> = ({ value, searchTerm = '', searchTrigger = 0, onMatchCountChange }) => {
  ...
  return (
    <GraphContext.Provider value={contextValue}>
      <div className="relative w-full h-full overflow-hidden bg-background select-none transition-colors duration-300 focus:outline-none" tabIndex={0} onKeyDown={handleKeyDown}>
        <div className="absolute top-4 right-4 flex flex-col gap-2 z-50 bg-muted/95 border border-border p-1 rounded-lg shadow-2xl backdrop-blur-md">
          <button onClick={handleZoomIn} className="p-2 hover:bg-muted-foreground rounded text-muted-foreground hover:text-foreground" title="Zoom In (+)">
            <ZoomIn size={16} />
          </button>
          <button onClick={handleZoomOut} className="p-2 hover:bg-muted-foreground rounded text-muted-foreground hover:text-foreground" title="Zoom Out (-)">
            <ZoomOut size={16} />
          </button>
          <button onClick={handleReset} className="p-2 hover:bg-muted-foreground rounded text-muted-foreground hover:text-foreground" title="Reset Scale (0)">
            <RotateCcw size={16} />
          </button>
          <button onClick={handleFitScreen} className="p-2 hover:bg-muted-foreground rounded text-muted-foreground hover:text-foreground" title="Fit to Screen">
            <Shrink size={16} />
          </button>
          
          <div className="h-px bg-border mx-1 my-0.5" />
          
          <button onClick={handleExpandAll} className="p-2 hover:bg-muted-foreground rounded text-muted-foreground hover:text-foreground" title="Expand All Nodes">
            <ChevronsDown size={16} />
          </button>
           <button onClick={handleCollapseAll} className="p-2 hover:bg-muted-foreground rounded text-muted-foreground hover:text-foreground" title="Collapse All Nodes">
            <ChevronsUp size={16} />
          </button>
        </div>
