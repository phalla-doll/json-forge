...
export function SchemaModal({
  ...
  const [results, setResults] = useState<Partial<Record<SchemaTarget, string>>>(
    {},
  );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      onRemainingChange(r);
      if (r === 0) trackEvent("ai_quota_exhausted", { endpoint: "types" });
      ...
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <HugeiconsIcon icon={DocumentCodeIcon} className="size-5" />
            Generate schema from JSON
          </DialogTitle>
          <DialogDescription>
            Pick a target — the model reads your current JSON and returns a
            schema in that language.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="min-h-[14rem] rounded-md border border-border bg-muted/30 p-3">
            {loading === target ? (
              ...
              <pre className="max-h-80 overflow-auto font-mono text-xs leading-relaxed text-foreground whitespace-pre-wrap break-all">
                {code}
              </pre>
            ) : (
              <div className="flex h-56 flex-col items-center justify-center gap-2 text-sm text-muted-foreground">
                ...
                <span>
                  Click <strong>Generate</strong> to produce {TARGETS.find((t) => t.value === target)?.label}.
                </span>
              </div>
            )}
          </div>
          ...
          <div className="flex items-center justify-between gap-2">
            <span className="text-xs text-muted-foreground">
              {remaining !== null ? `${remaining}/5 AI requests left this hour` : ""}
            </span>
