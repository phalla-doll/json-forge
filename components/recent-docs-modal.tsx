...
export function RecentDocsModal({
    ...
    const handleOpen = async (doc: RecentDoc, index: number) => {
        ...
        if (body === null) {
            ...
            setRecents(
                (prev) => prev?.filter((d) => d.hash !== doc.hash) ?? [],
            );
            return;
        }
        ...
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Local-only history. Up to 10 distinct documents are kept
                        in this browser.
                    </DialogDescription>
                </DialogHeader>
                ...
                {loading ? (
                    ...
                    <ul className="flex max-h-96 flex-col gap-1 overflow-y-auto">
                        {(recents ?? []).map((doc, i) => (
                            <li
                                key={doc.hash}
                                className="group hover:border-border hover:bg-muted/50 flex items-center gap-2 rounded-md border border-transparent px-2 py-2"
                            >
                                <button
                                    type="button"
                                    onClick={() => handleOpen(doc, i)}
                                    className="focus-visible:ring-ring flex flex-1 flex-col items-start gap-0.5 rounded text-left focus-visible:ring-2 focus-visible:outline-none"
                                >
                                    <span className="text-foreground line-clamp-1 font-mono text-xs font-medium">
                                        {doc.title}
                                    </span>
                                    <span className="text-muted-foreground flex items-center gap-2 text-[10px]">
                                        <span>
                                            {formatRelative(doc.savedAt)}
                                        </span>
                                        <span>·</span>
