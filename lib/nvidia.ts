...
const TARGET_INSTRUCTIONS: Record<SchemaTarget, string> = {
    ...
    zod: 'Generate a Zod schema (compatible with `zod` v3) that validates the JSON shape. Start the file with `import { z } from "zod";`. Use `z.union`, `z.array`, `z.object` as appropriate. Export the root schema as `RootSchema` and infer a `Root` type via `z.infer`.',
    "json-schema":
