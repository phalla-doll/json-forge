...
export const JsonMermaidView: React.FC<JsonMermaidViewProps> = ({
  value,
  theme,
}) => {
  ...
  const generateMermaidSyntax = useMemo(() => {
    try {
      ...
      if (nodeCount > MAX_NODES) {
        setError(
          `Data is too large for diagram view (${nodeCount}+ nodes). Showing partial tree.`,
        );
      } else {
        setError(null);
      }
        ...
        <div className="flex flex-col gap-1 border-b border-accents-2 pb-2 mb-1">
          <button
            onClick={() => {
              ...
              trackEvent("diagram_switch", { type: "mindmap" });
            }}
          ...
          <button
            onClick={() => {
              ...
              trackEvent("diagram_switch", { type: "graph" });
            }}
