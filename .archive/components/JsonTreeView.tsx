...
const GraphNode: React.FC<{
  ...
  ({ name, value, depth = 0, isLast = true, path = "$", expansionConfig }) => {
    // Determine type for initialization logic
    const type = getDataType(value);
    const isExpandable = type === "object" || type === "array";

    const { showTooltip, hideTooltip, globalAction, searchTerm, focusNode } =
    ...
    const TypeIcon = () => {
      switch (type) {
        ...
        case "array":
          return (
            <List size={14} className="text-yellow-600 dark:text-yellow-400" />
          );
        case "string":
          return (
            <Type size={14} className="text-green-600 dark:text-green-400" />
          );
        case "number":
          return (
        ...
        default:
          return <div className="w-3.5 h-3.5 rounded-full bg-accents-4" />;
      }
    };
    ...
    const renderValue = () => {
      if (isExpandable) {
        ...
        return (
          <span className="text-accents-4 text-[10px] ml-2">
          ...
          >
            ...
            <div className="flex flex-col">
              <div className="flex items-center">
                {name && (
                  <span className="font-semibold text-xs text-accents-8 mr-1">
                    {name}
                  </span>
                )}
                {!name && (
                  <span className="font-semibold text-xs text-accents-5 italic">
                    root
                  </span>
                )}
              </div>
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-accents-2 pb-2">
        <span className="font-semibold text-accents-8 break-all">
          {data.name || "root"}
        </span>
      ...
      <div className="flex flex-col gap-1">
        <span className="text-accents-4 text-[10px] uppercase tracking-wider">
          Path
        </span>
...
export const JsonGraphView: React.FC<JsonGraphViewProps> = ({
  value,
  searchTerm = "",
  searchTrigger = 0,
  onMatchCountChange,
  ...
  return (
    <GraphContext.Provider value={contextValue}>
      <div
        className="relative w-full h-full overflow-hidden bg-background select-none transition-colors duration-300 focus:outline-none"
        tabIndex={0}
      ...
      >
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2 z-50 bg-accents-1/95 border border-accents-2 p-1 rounded-lg shadow-2xl backdrop-blur-md">
          <button
            ...
            className="p-2 hover:bg-accents-3 rounded text-accents-5 hover:text-accents-8"
            title="Zoom In (+)"
          >
            <ZoomIn size={16} />
          </button>
        ...
        >
          <div
            style={{
              ...
              transformOrigin: "0 0",
              transition:
                isPanningRef.current || isWheelingRef.current
                  ? "none"
        ...
        <div className="absolute bottom-4 left-4 right-4 md:right-auto bg-accents-1/80 backdrop-blur border border-accents-2 px-3 py-2 rounded-md flex flex-wrap gap-x-4 gap-y-2 text-[10px] text-accents-5 pointer-events-none justify-center md:justify-start">
          <div className="flex items-center gap-1">
            <Box size={10} className="text-blue-600 dark:text-blue-400" />{" "}
            Object
          </div>
          <div className="flex items-center gap-1">
            <List size={10} className="text-yellow-600 dark:text-yellow-400" />{" "}
            Array
          </div>
          <div className="flex items-center gap-1">
            <Type size={10} className="text-green-600 dark:text-green-400" />{" "}
            String
          </div>
          <div className="flex items-center gap-1">
            <Hash size={10} className="text-orange-600 dark:text-orange-400" />{" "}
            Number
          </div>
