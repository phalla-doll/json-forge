...
export const JsonTableView: React.FC<JsonTableViewProps> = ({
  ...
  const renderCell = (cellValue: any, onClickPath?: (string | number)[]) => {
    ...
    if (cellValue === undefined)
      return <span className="text-accents-3 text-[10px] italic"></span>;
    ...
    if (isObj) {
      ...
      return (
        ...
        >
          <FolderOpen
            size={10}
            className="text-accents-5 group-hover:text-accents-8"
          />
          {label}
        </button>
      );
    ...
    if (typeof cellValue === "boolean") {
      return (
        <span
          className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${cellValue ? "bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400" : "bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400"}`}
        >
          {String(cellValue)}
        </span>
      );
    ...
    if (str.length > 80) {
      return (
        <span className="text-accents-6" title={str}>
          {str.substring(0, 80)}...
        </span>
      );
    }
      ...
      <div className="flex-1 overflow-auto bg-background relative">
        {currentType === "array" &&
          (() => {
            ...
            if (arr.length === 0)
              return (
                <div className="h-full flex items-center justify-center text-accents-4 text-sm">
                  Empty Array
                </div>
              );
            ...
            if (filteredArr.length === 0)
              return (
                <div className="h-full flex flex-col items-center justify-center text-accents-5 gap-2">
                  <Filter size={24} />
              ...
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 z-10">
                  <tr className="bg-accents-1 shadow-sm">
                    <th className="p-2 border-b border-accents-2 font-medium text-accents-5 text-[11px] uppercase tracking-wider w-12 text-center sticky left-0 bg-accents-1 border-r">
                      #
                    </th>
                    {headers.map((h) => (
                      <th
                        key={h}
                        className="p-2 border-b border-accents-2 font-medium text-accents-5 text-[11px] uppercase tracking-wider min-w-[120px] whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-accents-2">
                  {filteredArr.map((row, idx) => {
                    ...
                    return (
                      <tr
                        key={idx}
                        className="hover:bg-accents-1/50 transition-colors group"
                      >
                ...
                <div className="w-full max-w-3xl border border-accents-2 rounded-lg overflow-hidden shadow-sm bg-background">
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-accents-1">
                      <tr>
                        <th className="p-2 border-b border-accents-2 font-medium text-accents-5 text-[11px] uppercase tracking-wider w-1/3 border-r">
                          Key
                        </th>
                        <th className="p-2 border-b border-accents-2 font-medium text-accents-5 text-[11px] uppercase tracking-wider">
                          Value
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-accents-2">
                      {filteredKeys.map((key) => (
                        <tr
                          key={key}
                          className="hover:bg-accents-1/50 transition-colors"
                        >
                          <td className="p-2 font-semibold text-accents-7 font-mono text-xs align-top border-r border-accents-2 select-text">
                            {key}
                          </td>
          ...
          <div className="h-full flex flex-col items-center justify-center p-8 bg-background">
            <div className="bg-accents-1 p-8 rounded-xl border border-accents-2 shadow-sm text-center max-w-md w-full relative">
              <button
                ...
                className="absolute top-4 left-4 p-1 hover:bg-accents-2 rounded text-accents-5"
              >
                <ArrowLeft size={16} />
              </button>
              <div className="mb-4 text-accents-5 text-xs uppercase tracking-wider font-semibold border-b border-accents-2 pb-2">
                Value
              </div>
              <div className="text-xl font-mono text-accents-8 break-all select-all max-h-60 overflow-y-auto">
                {String(currentData)}
              </div>
              <div className="mt-4 pt-2 text-[10px] text-accents-4 border-t border-accents-2 flex justify-between">
                <span>
                  Type:{" "}
                  <span className="font-semibold text-accents-6">
                    {typeof currentData}
                  </span>
                </span>
              </div>
